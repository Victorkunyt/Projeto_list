...
import { CategoryUpdateController } from "../controllers/Category/PutCategory_controller";
...
export async function routesCategory(fastify: FastifyInstance, options: FastifyPluginOptions) {
  fastify.register(async function(fastify) {
    ...
    fastify.put("/putCategory", async (request,reply) => {
      return new CategoryUpdateController(prisma).handle(request,reply)
    })
